...
const {Course,Faculty,Department,Lecturer, Student,} = require("../config/model_sync");
...
exports.course_page = asyncHandler(async(req,res, next,) => {
    ...
    const courseDepartment = await Department.findByPk(courseLecturer.departmentId);
    ...
    courseRaw.courseCode = "0" + courseFaculty.id + courseDepartment.id + courseLecturer.id + courseRaw.id;
    res.render(
        'course_page',
        {
    ...
    if (req.method === "POST") {
        ...
        if (Array.isArray(course.greatGrandParent)) {
            ...
            if (selectedDepartmentLecturers.length < 1){
                ...
                // Check for an atleast a lectuerer with no course assigned to it
                const availableLecturers =[];
                for await (const lecturer of selectedDepartmentLecturers) {
                    const obj = await lecturer.getCourse();
                    if(!obj)
                        availableLecturers.push(lecturer);
                }    
        
                if(availableLecturers.length >0){
                    // There exist at this point atleast a lecturer 
                    // that that is yet to be assigned a course
            ...
            const lecturerList = checkAssociatedModelInstances(selectedDepartmentlecturers, [selectedLecturer]);
    ...
    asyncHandler(async (req, res, next) =>{
        ...
        if (!errors.isEmpty()) {
            ...
            if (courseWithSameName) {
                const courseLecturerList = await get_Obj_array_from_id_array([course.lecturerId], Lecturer);    
                lecturerList= checkAssociatedModelInstances(lecturerList, courseLecturerList);
                
                 res.render(
                    'course_already_existing',
                    {
    ...
    asyncHandler(async (req, res, next) =>{
        ...
        if (!errors.isEmpty()) {
            ...
            if (courseWithSameName && courseWithSameName.id != req.params.id) {
                const workedLecturerList = checkAssociatedModelInstances(lecturerList, [course.lecturer]);
