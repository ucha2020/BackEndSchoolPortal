...
const {Faculty,Course,Student,Department,Lecturer} = require("../config/model_sync")
const asyncHandler = require("express-async-handler");
const {body, validationResult} = require("express-validator");
const {get_Obj_array_from_id_array,checkAssociatedModelInstances} = require("../resources/libary")
...
exports.faculty_update_form =  asyncHandler(async(req,res, next,) => {   
    ...
    if (req.method === "GET") { 
        faculty = await Faculty.findByPk(
            req.params.id,
            {
                include:[Department],
            }
        ...
        facultyDepartmentList = await get_Obj_array_from_id_array(faculty.departmentIds, Department);
        facultyCourseList = await get_Obj_array_from_id_array(faculty.courseIds, Course);
        facultyLecturerList = await get_Obj_array_from_id_array(faculty.lecturerIds, Lecturer);
        facultyStudentList = await get_Obj_array_from_id_array(faculty.studentIds, Student);
    }  

     //Add checked plag to each of the departments associated to the faculty from departmentList
     const workedDepartmentList = checkAssociatedModelInstances(departmentList, facultyDepartmentList);
    ...
    asyncHandler(async (req, res, next) =>{
        ...
        if (!errors.isEmpty()) {
            ...
            const facultyWithSameName = await Faculty.findOne({
                ...
                const facultyDepartmentList = await get_Obj_array_from_id_array(faculty.departmentIds, Department);
